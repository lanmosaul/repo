...
public final class DateUtil {
	...
	public static void main(String[] args) throws Exception{
    	...
    	Date start=DateUtil.convertStringToDate("HH:mm",null);
		Date end = DateUtil.convertStringToDate("HH:mm",null);
        System.out.println(!DateUtil.judge(start, end, nowHHmm));
    }
    ...
    /***
     * 获取指定日期为星期几
     * 1:星期天 2:星期一 3:星期二 ... 7:星期六
     * @param date
     * @return
     * @throws Exception
     */
	public static int getDayOfWeek (String dateStr) throws Exception {
        Date date = convertStringToDate(FORMAT_YYYY_MM_DD, dateStr);
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(date);
        int week = calendar.get(Calendar.DAY_OF_WEEK);
        return week;
	...
	/***
	 * 判断目标时间是不是在时间范围之内，包含起始时间和结束时间
	 * @param start
	 * @param end
	 * @param target
	 * @return
	 */
	public static boolean isInDateRange(Date start, Date end, Date target) {
